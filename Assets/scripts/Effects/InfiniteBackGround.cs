...
namespace PixelCrew.Effects
{
    public class InfiniteBackGround : MonoBehaviour
    {
        ...
        private void InstantiateContainer(float centerX)
        {
            var newBounds = new Bounds(new Vector3(centerX, _containerBounds.center.y), _containerBounds.size);
            _allBounds.Encapsulate(newBounds);

            _boundsToTransformDelta = transform.position - _allBounds.center;
            var newContainerXPos = centerX + _containerDelta.x;
